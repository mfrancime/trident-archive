...
export async function synthesize(data) {
  ...
  const health = Object.entries(data.sources).map(([name, src]) => {
    ...
    const noKey = src.status === 'no_key' || src.status === 'no_credentials';
    const rateLimited = src.status === 'rate_limited' || Boolean(src.rateLimited);
    const degraded = src.status === 'degraded';
    return {
      n: name,
      err: hasError,
      ...
      rateLimited,
      degraded,
      status: noKey ? 'no_key' : hasError ? 'error' : rateLimited ? 'rate_limited' : degraded ? 'degraded' : 'ok',
    };
  });
