...
export async function fullBriefing() {
  ...
  // Count sources with real data vs degraded (no_key, rate_limited, empty)
  const okSources = sources.filter(s => s.status === 'ok');
  const degraded = okSources.filter(s => {
    const d = s.data;
    return d?.status === 'no_key' || d?.status === 'no_credentials' || d?.status === 'rate_limited' || Boolean(d?.error);
  });
  const reallyOk = okSources.length - degraded.length;
  ...
  const output = {
    crucix: {
      ...
      degradedList: degraded.map(s => ({ name: s.name, reason: s.data?.status || s.data?.error || 'unknown' })),
    },
