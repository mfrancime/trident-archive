...
export async function searchEvents(query = '', opts = {}) {
  ...
  // If no query, use broad geopolitical terms (parentheses required for OR queries)
...
export async function briefing() {
  ...
  if (all?.error || all?.rateLimited || all?.rawText || all?._nonJson) {
    return {
      source: 'GDELT',
      ...
      status: all?.rateLimited ? 'rate_limited' : 'error',
      message: all?.error || all?.rawText || 'Non-JSON response from GDELT API',
    };
  }
